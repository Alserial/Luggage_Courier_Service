...
1. 审核云函数和审核后台
   - 缺 `item-request-review`：将 `item_requests.reviewStatus` 从 `pending` 改为 `approved`、`rejected` 或 `manual_review`。
   - 缺 `trip-verify`：审核行程真实性、航班号、票据证据和核验状态。
   - 缺运营端最小页面或 CloudBase CMS 配置，用于查看待审核记录、填写审核原因、写入审计日志。

2. 前端真实数据联调
   - 行程列表需要读取当前用户的 `trips`。
   - 需求列表需要读取当前用户的 `item_requests`。
...
4. 证据文件上传链路
   - 前端需要把图片、视频上传到 CloudBase storage。
...
3. 站内聊天和证据化聊天
   - 当前没有聊天功能。
   - 后续如加入聊天，应支持将关键聊天记录归档为 `in_app_chat` 证据。
   - 聊天不得替代订单状态和证据记录。
...
1. 先补 `item-request-review` 和 `trip-verify`，让审核状态能真实流转。
2. 再补行程、需求、订单列表读取真实 CloudBase 数据。
3. 接入云存储上传，让证据从 `fileCount` fallback 变成真实 fileId。
4. 补管理员争议裁决和服务费退款 mock，闭合争议路径。
5. 最后补交接码生成、证据门槛状态机、通知、评价和测试数据脚本。
